...
import { EtatTache } from 'src/app/model/modelGMAO/etatTache.model';
import { EtatTacheService } from 'src/app/services/serviceBD_GMAO/etat-tache.service';

@Component({
  selector: 'app-etat-tache',
  templateUrl: './etat-tache.component.html',
  styleUrls: ['./etat-tache.component.scss']
})
export class EtatTacheComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    private categSe: EtatTacheService) {
    ...
    this.categSe.getAll(this.request)
      .subscribe(
        res => {
          ...
          if (resultat.status) {
            this.etatTaches = resultat.resultat.docs
  ...
  enregistrerEtatTache() {
    if (!this.fnctModel.controleInputs(this.erreurEtatTache, this.etatTache,this.tabLibelle, 'libelle')) {
    ...
    this.isLoading = true
    this.categSe.create(this.etatTache, this.request)
      .subscribe(
        res => {
          this.isLoading = false
          let resultat: any = res
          if (resultat.status) {
            this.getAllParametres()
            this.getEtatTaches()
            this.notificationToast.showSuccess("Votre etatTache est bien enregistrée !")
          }
        },
        error => {
          this.isLoading = false
  ...
  id = ""
  modifierEtatTache() {
    if (!this.fnctModel.controleInputsModifer(this.erreurEtatTache, this.etatTache,this.tabLibelle)) {
      this.notificationToast.showError("Veuillez remplir les champs obligatoires !")
      return
    }
    if (this.isLoading) {
    ...
    this.isLoading = true
    this.categSe.update(this.id, this.etatTache, this.request)
      .subscribe(
        res => {
          this.isLoading = false
          let resultat: any = res
          if (resultat.status) {
            this.getAllParametres()
            this.getEtatTaches()
            this.notificationToast.showSuccess("Votre etatTache est bien modifiée !")
          }
        },
        error => {
          this.isLoading = false
  ...
  openModalDelete(id, params2) {
    ...
    this.params1Delete = "La etatTache"
      ...
      .subscribe(
        res => {
          ...
          if (resultat.status) {
            ...
            for (let item of this.allEtatTaches) {
  ...
  //pour rendre chaine to HTML
  getDataToHtml(etatTaches) {
    return this.fctList.getDataToHtml(this.etatTaches)
  }
